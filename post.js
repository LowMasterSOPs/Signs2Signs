...
// ---------- Partials (absolute paths so they load under /blog/slug) ----------
...
function getSlugFromPath() {
  // supports /blog/slug and /blog/slug/
...
function setupCommentForm(postId) {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    // Basic client-side checks (keep it friendly)
    ...
    try {
      ...
      console.error("[comments] exception:", e2)
      alert("Error saving comment: " + e2.message)
    }
  })
}
...
async function renderPost() {
  ...
  if (!slug) {
    postEl.innerHTML = "<p class='empty'>No post specified.</p>"
    return
  }
  ...
  if (shareBox) {
    ...
    if (tw) tw.href = `https://twitter.com/intent/tweet?url=${encodeURIComponent(currentUrl)}&text=${encodeURIComponent(post.title ?? "Check this out!")}`
  }

  // Comments
  await loadComments(post.id)     // post.id must match comments.post_id type (bigint or uuid)
